...
message PWebsocketQuoteUpdateV1Args {
  ...
  // Optional PropAMM metadata.
  // Pairs affected by this update. Each entry is 40 bytes: token0 || token1.
  repeated bytes asset_pairs = 6;
  // 20-byte address a taker must touch to receive a quote.
  bytes quote_address = 7;
  // Pool addresses whose swaps require this update. Each entry is 20 bytes.
