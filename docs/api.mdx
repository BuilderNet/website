...
* **Conditional:** to land only on the condition that another transaction reading the update is also included in the same block. Conditional updates are not guaranteed to be at the top of the block. This is an experimental gas savings feature subject to change. Currently, inclusion is not triggered by reads that occur in reverting call frames.
...
The mode is set per API key, which can be granted on request. This requirement may be dropped in future. Operators needing both modes can request multiple keys.
